...
public class ZdalConfigurationLoader {
    ...
    private synchronized Map<String, ZdalConfig> getZdalConfigurationFromLocal(String appName,
        ...
        File configurationFile = new File(configPath, MessageFormat.format(Constants.LOCAL_CONFIG_FILENAME_SUFFIX, appName, dbMode));
        if (configurationFile.exists() && configurationFile.isFile()) {
        	// Spring会将绝对路径前面的 / 去掉
//            zdalConfigurationFilePathList.add(configurationFile.getAbsolutePath());
        	zdalConfigurationFilePathList.add(configurationFile.getPath());
        }
        configurationFile = new File(configPath, MessageFormat.format(Constants.LOCAL_RULE_CONFIG_FILENAME_SUFFIX, appName, dbMode));
        if (configurationFile.exists() && configurationFile.isFile()) {
//            zdalConfigurationFilePathList.add(configurationFile.getAbsolutePath());
        	zdalConfigurationFilePathList.add(configurationFile.getPath());
        }
        if (zdalConfigurationFilePathList.isEmpty()) {
            throw new ZdalConfigException(
                "ERROR ## There is no local Zdal configuration files for " + appName + " to initialize ZdalDataSource.");
        }
        return loadZdalConfigurationContext(zdalConfigurationFilePathList.toArray(new String[zdalConfigurationFilePathList.size()]), appName, dbMode);
    }
