 ...
 * 
 * @author 伯牙
 * @version $Id: ZdalOracleSchemaStatVisitor.java, v 0.1 2012-11-17 下午3:57:22 Exp $
 */
public class ZdalOracleSchemaStatVisitor extends ZdalSchemaStatVisitor implements OracleASTVisitor {
    ...
    public ZdalOracleSchemaStatVisitor() {
    ...
    public boolean visit(SQLIdentifierExpr x) {
        if ("ROWNUM".equalsIgnoreCase(x.getName()) && x.getParent() instanceof SQLBinaryOpExpr) {
            SQLBinaryOpExpr parent = (SQLBinaryOpExpr) x.getParent();
            if (parent.getRight() instanceof SQLVariantRefExpr) {
                SQLVariantRefExpr right = (SQLVariantRefExpr) parent.getRight();
                BindVarCondition bindVarCondition = new BindVarCondition();
                bindVarCondition.setTableName(getCurrentTable());
                if (parent.getOperator() == SQLBinaryOperator.GreaterThan
                    || parent.getOperator() == SQLBinaryOperator.GreaterThanOrEqual) {
                    bindVarCondition.setColumnName(OFFSET);
                } else if (parent.getOperator() == SQLBinaryOperator.LessThan
                           || parent.getOperator() == SQLBinaryOperator.LessThanOrEqual) {
                    bindVarCondition.setColumnName(ROWCOUNT);
                }
                bindVarCondition.setOperator(parent.getOperator().name);
                bindVarCondition.setIndex(right.getIndex());
                rownums.add(bindVarCondition);
            } else if (parent.getRight() instanceof SQLIntegerExpr) {
                SQLIntegerExpr right = (SQLIntegerExpr) parent.getRight();
                BindVarCondition bindVarCondition = new BindVarCondition();
                bindVarCondition.setTableName(getCurrentTable());
                if (parent.getOperator() == SQLBinaryOperator.GreaterThan
                    || parent.getOperator() == SQLBinaryOperator.GreaterThanOrEqual) {
